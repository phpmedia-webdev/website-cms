...
- **Completed:**
  - **GPUM merged stream — duplicate fix:** `getMemberMessageCenterMergedStream` (`gpum-member-stream.ts`) omits activity **`notification`** rows for **`message`/`message`** when a **support** `conversation_head` exists (same pattern as admin: no parallel **Message** + **Support** lines).
  - **GPUM client — `MemberActivityStream` (`/members/messages`):** Filters, date presets, search, reset; inline thread fetch with **`enrichAuthors=1`**; transcript **oldest-first** with **scroll-to-bottom**; **staff/support on the left** (slate icon) and **member on the right** (blue icon); compact thread block (no inner bordered card; `border-t` separator); **Send reply** primary (blue); **Message support** composer **Send** right-aligned with Cancel.
  - **Transcript sublines (GPUM):** `formatGpumThreadSubline` — outgoing **`Member - {name} - {datetime}`** using **`memberContactNames`** (first+last, else full name, else email); incoming staff **`{roleLabel}, {displayName}, {datetime}`** from existing author enrichment; system fallback.
  - **`thread-message-author-enrichment.ts`:** Adds **`memberContactNames`** (GPUM name order) while keeping admin **`contactNames`** (full name first). **`GET /api/conversation-threads/[threadId]/messages?enrichAuthors=1`** returns **`memberContactNames`** with **`authors`** / **`contactNames`**.
  - **Admin Message Center / CRM:** `DashboardActivityStream` and related stream/timeline/thread work; **`/admin/dashboard/message-center`** contact label when `contact_id`; CRM contact layout/thread wiring; **`GET /api/admin/message-center`** extensions; **`/api/admin/message-center/notes`**, **`notes/[id]`**, **`broadcast`** routes; **`MessageCenterBroadcastPanel`** updates; **`mag-thread-policy`** (member read gate, etc.); **`admin-stream`**, **`admin-filters`**, **`conversation-threads`**, **`crm`**, **`contact-notifications-timeline`**, **`notifications-timeline`** API adjustments; **`getAdminRoleDisplayLabel`** in **`role-mapping.ts`**.
  - **Product comments (API surface):** **`/api/product/comments/[id]`** + **`product-comment-messages.ts`** (wiring toward Phase 18C-style threads).
  - **Docs:** **`sessionlog.md`** — GPUM checklist 0–3 marked shipped, expanded **Next up** (1–5), **Recently shipped** admin/GPUM bullets; **`planlog.md`** Phase 18C status (GPUM read API + UI v1); **`messages-and-notifications-wiring.md`** touch; **`reference/plan-gpum-message-center-mvp.md`** added.
  - **`mvt.md`:** Message Center module paths/version note updated for GPUM + enrichment.

### 2026-03-26 23:53 CT — Admin Message Center: GPUM dedupe, search text, dashboard fixes

- **Context for Next Session:** **Product (open):** Decide whether GPUM should move toward **SMS-style chat UI** (single thread, chronological bubbles) vs keeping the **unified activity + thread-head** model on admin dashboard — discussion started; no spec change yet. **Tech debt / follow-up:** Optional **expandable support row** or dedicated **chat panel** so admins see full thread like members without relying on search. **DB:** Unchanged this session (no new migrations). **Test:** `/admin/dashboard` loads; Message Center tab — one **support** row per GPUM thread without duplicate **`message`** timeline lines for the same contact; search still matches older thread text via **`threadSearchText`**. Member **`/api/members/message-center`** + activity stream changes in tree — smoke member dashboard/messages if touched.
- **Completed:**
  - **`getAdminMessageCenterStream` / `admin-stream.ts`:** Contact-scoped merge improvements (support thread attribution, caps, `threadSearchText` batch enrichment for support threads for admin search); **dedupe** — skip **`contact_notifications_timeline`** rows with **`kind: message`** when that contact already has a **support** thread row in the stream (avoids double-listing GPUM next to thread heads).
  - **`conversation-threads.ts` / `crm.ts` / `projects.ts` / `contact-notifications-timeline` / `date-range.ts`:** Support-thread listing, task follower union, notes/thread helpers, message-center fetch caps — as in working tree (GPUM parity work).
  - **`DashboardActivityStream`:** Search uses **`threadSearchText`**; removed experimental **“Earlier:”** subline; **fixed** stray **`supportThreadEarlierLine`** reference that caused **`ReferenceError`** and **500** on dashboard.
  - **Dashboard SSR:** **`getAdminMessageCenterStream(120, …)`** initial fetch (was 60) so Activity tab is less likely to miss threads when using SSR-only items.
  - **CRM / members / APIs (in tree):** Contact record projects query fix (**`projects.name`**), member message-center route + **`MemberActivityStream`** fetch path, admin message-center **mark-read** batch route, **`/admin/dashboard/message-center`** page + redirect, sidebar/feature gates, bulk CRM status UX pieces — per diff vs last push.
  - **Docs touched in tree:** `sessionlog`, `planlog`, `plan-message-center-roadmap` — synced with messaging progress.

### 2026-03-26 16:44 CT — Message Center (admin), MAG messaging controls, task reminders/calendar hooks

- **Context for Next Session:** **DB:** Primary is current through **`214`**, including **`197`**/**`198`**/**`200`** (All Tasks + project numbers) and **`207`**–**`214`** (tasks/contact/standalone/reminders/Message Center). Forks: run missing **`supabase/migrations/`** scripts in numeric order. **Test:** Admin **Dashboard → Message Center** tab — filters, thread rows, unread badge; **CRM → Memberships → MAG** — member conversation switch; **POST** thread messages on **`mag_group`** with `allow_conversations` on/off; member **Profile** — MAG community block when member has MAGs. **Next (sessionlog §3):** GPUM merged read API + UI; bulk mark-read; timeline unread if desired; per‑MAG “notify admin on opt-in” if product still wants it.
- **Completed:**
...
- **Completed:**
  - **DB:** **207** / **208** — **applied** (primary Supabase, 2026-03-25). **207** — `tasks.contact_id` → `crm_contacts`, index, comment; **`get_tasks_dynamic` / `get_task_by_id_dynamic`** include **`contact_id`**. **208** — **`ALTER COLUMN project_id DROP NOT NULL`**; **`get_tasks_dynamic`** uses **LEFT JOIN** `projects`, filter for unassigned + archived rules.
  - **App / lib:** **`projects.ts`** — `Task` / **`TaskUpdate`** **`contact_id`** + **`project_id` null**; **`normalizeTaskFromRpc`**; **`updateTask`** / **`maybeExtendProjectEndDate`** (skip null project); **`createTask`** extend only when project set. **`crm.ts`** — **`formatCrmContactDisplayName`**. **`PUT /api/tasks/[id]`**, **`POST /api/projects/[id]/tasks`**, **`members/support`** — **`contact_id`** where applicable.
  - **Routes / nav:** **`taskDetailPath` / `taskEditPath`**; **`/admin/projects/tasks/[taskId]`** + **`/edit`** (unassigned canonical); project-scoped task pages **redirect** when URL project ≠ task; **`AllTasksListClient`** + **`projects/page`** null-safe **`project_id`**; **`TaskEditClient`** — project select **No project**, persisted vs draft **Cancel** URLs.
  - **UI components:** **`TaskAssigneesDetailCard`** / **`TaskFollowersSection`** — linked contact, contacts-only modal, **`DirectoryParticipantPicker`** **`contactsOnly`** + **`maxSelections={1}`** for contact; **`AutoSuggestMulti`** **`maxSelections`**; assignee modal hint when directory has no team rows.
  - **Docs:** **[sessionlog.md](./sessionlog.md)** — §1 task-role Customizer + creator assign + task edit polish bullets; §3 **hide from client** edge case.

### 2026-03-25 16:36 CT — Projects: Customizer slugs (status/type/member roles), cover + rate, edit header actions

- **Context for Next Session:** **Migrations 203–206** — **applied** on tracked tenant schema(s); [sessionlog.md](./sessionlog.md) Manual SQL outstanding **no longer lists** them. **Other forks/envs:** run **`203`–`206`** in SQL Editor if still on legacy `status_term_id` / `role_term_id` columns. **Remaining Manual SQL:** see sessionlog (**200**, **198**, **197**, etc.). **Test:** Admin **projects** list (`status_slug` query + pills), **new/edit** (Customizer status/type, cover, hourly rate, **Cancel**/**Save** on edit header with `form="project-edit-form"`), **detail** badges from slugs, **add member** (directory + `role_slug`), **Mark complete** (status slug `complete` / `completed` + `completed_date`). **`admin-task-list.ts`** / event-link helpers use **`project_status_slug`**. **Next:** Attachments tab; Message Center on project detail; align **support project** planlog bullet with **`project_type_slug`**; any external clients still sending `status_term_id` / `role_term_id` must switch to slugs.
- **Completed:**
  - **DB:** **203** — `projects.estimated_hourly_rate`; **204** — cover image path + storage policy notes; **205** — `project_status_slug`, `project_type_slug`; backfill from former term IDs; drop `status_term_id` / `project_type_term_id`; `get_projects_dynamic` / `get_project_by_id_dynamic` return slugs and filter via **`project_status_slug_filter`** (text); **206** — `project_members.role_slug`, backfill from terms, drop `role_term_id`.
  - **App / API:** `src/lib/supabase/projects.ts` — Customizer-only options for `project_status`, `project_type`, **`project_role`**; `ListProjectsFilters.status_slug`; members **`role_slug`**; **`normalizeProjectCustomizerSlug`**. **`GET/POST /api/projects`**, **`PUT /api/projects/[id]`**, **`POST .../members`** use slug fields; **`resolve-project-member-labels`** (and related) resolve roles by slug.
  - **Admin UI:** `ProjectEditClient` — slug-backed status/type, estimated rate, cover, directory **Add from directory**, header **Cancel** then **Save**; `ProjectDetailClient` / `ProjectNewClient` / projects **`page.tsx`** list maps; **`admin-task-list.ts`** **`filterActiveProjectsForTaskList`** by status slug (e.g. exclude **`complete`**).

### 2026-03-25 11:14 CT — Session wrap: project detail polish, planned time migrations, git push policy

- **Context for Next Session:** **Migrations 201 + 202** — applied per developer records; [sessionlog.md](./sessionlog.md) Manual SQL outstanding no longer lists them. **Remaining Manual SQL:** see sessionlog (**200**, **198**, **197**, etc.). **Test:** Project **detail** — overview hero, date/progress metrics, client/team bento row, six tabs (Message Center + Deliverables placeholders). **Task** detail/edit — planned vs logged time; **All Tasks** / APIs — `planned_time` parity. **Git:** Pre-push build hook **removed**; `pnpm run setup-git-hooks` only deletes a legacy `.git/hooks/pre-push`. See `.cursor/rules/sessions.mdc` + `coding.mdc`. **Next:** Attachments / Message Center / Deliverables wiring; naming sweep Estimated→Planned; optional `proposed_time` removal migration.
- **Completed:**
  ...
  - **Docs:** [sessionlog.md](./sessionlog.md) Manual SQL + time-model notes; [planlog.md](./planlog.md) Phase 19 project detail tabs line.
