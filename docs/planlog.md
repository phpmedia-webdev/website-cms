...
- [ ] **Project Management (projects + tasks)** — step-by-step. Feature slug: **projects** (roles/gate step at end).
  ...
  - [x] **Schema / RPC — tasks `contact_id` (migration 207):** FK to **`crm_contacts`**; **`get_tasks_dynamic` / `get_task_by_id_dynamic`** return **`contact_id`**. Manual SQL per tenant.
  - [x] **Schema / RPC — tasks nullable `project_id` (migration 208):** Loose tasks; **`get_tasks_dynamic`** LEFT JOIN **`projects`**. Manual SQL per tenant.
