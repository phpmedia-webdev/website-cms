...
- [ ] **Project Management (projects + tasks)** — step-by-step. Feature slug: **projects** (roles/gate step at end).
  - [x] **Schema — projects:** Table `projects`: id, name, description, status_term_id, project_type_term_id, start_date, due_date, completed_date (nullable dates), end_date_extended (boolean), proposed_time, potential_sales, required_mag_id (FK → mags), contact_id, client_organization_id, archived_at, created_at, updated_at, created_by. RLS, indexes. **199** renames former `proposed_*` dates and adds `completed_date`; run SQL on each tenant.
  ...
  - [x] **Admin UI — project create/edit:** Form: name, description, status, start/due/completed dates, potential_sales, required_mag_id, taxonomy (categories/tags). **Edit (Mar 2026):** Save requires a **client** (contact or organization); **Client & members** card — 25%/75% columns, **Set client** dialog, **Add member** = team + CRM contact only.
