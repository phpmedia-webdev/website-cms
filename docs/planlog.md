...
**Status:** In progress — admin **Message Center** v1 (thread heads + timeline union, MAG controls, unread) shipped (**214** + app). **GPUM** merged stream + dedicated member UI still open. Spec alignment: [prd.md — Shared identity UX (forks)](./prd.md#shared-identity-ux-forks) (auth unchanged). **No CRM history backfill required in dev** unless desired later.
...
- [ ] **Read APIs — GPUM:** Merged stream + cursor pagination + **`admin_only`** visibility filtering on member routes (still open).
- [ ] **UI — contact detail:** **v1 shipped:** `ContactNotificationsTimelineSection` on Notes tab (list + add `staff_note`). **Remaining:** **mini Message Center** — contact-filtered stream (same model as dashboard `getAdminMessageCenterStream` / thread heads ∪ timeline); drill-down from CRM; merge/filters, `kind` categories, deep links ([sessionlog §3](./sessionlog.md)).
...
- [ ] **UI — GPUM Messages / notifications:** Dedicated member area stream + thread drill-in (support/DM/MAG); filters; parity with admin merged model.
