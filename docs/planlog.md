...
- **Number sequences (migration 194 + 200):** `number_sequences.sequence_year` (UTC); invoice/order and task triggers reset the `NNNNN` segment to `start_number` when the UTC calendar year changes (Jan 1 UTC). Formats `INV-YYYY-NNNNN` / `TASK-YYYY-NNNNN` / `PROJ-YYYY-NNNNN` (**200** `projects_assign_project_number`). Run SQL in Supabase SQL Editor per tenant schema.
...
**Status:** In progress — admin **Message Center** v1 + **CRM contact** scoped stream + full-page **`/admin/dashboard/message-center`** shipped (**214** + app). **GPUM** merged stream + **`/members/messages`** (`MemberActivityStream`) shipped at **functional v1**; **member thread unread + `PATCH .../read` gated by `assertMemberCanReadThread`** (2026-03-27); **cursor keyset pagination** on `GET /api/members/message-center` + **Load more** in UI (2026-03-27); **open:** manual QA pass, timeline/thread cutover + edge-case checklist ([sessionlog §3](./sessionlog.md) Phase **5**). Spec alignment: [prd.md — Shared identity UX (forks)](./prd.md#shared-identity-ux-forks) (auth unchanged). **No CRM history backfill required in dev** unless desired later.

**Next build focus:** GPUM **Phase 4–5** ([sessionlog §3](./sessionlog.md)); **tenant migrations** for **contact timeline** + **thread** tables (and RLS) **before** heavy task-detail comments UI — comments should target **thread storage** per [prd-technical § Phase 18C](./prd-technical.md#phase-18c-directory-and-messaging). Directory picker wiring (Projects/Tasks) can proceed in parallel where not blocked.
...
- [x] **Wire pickers — Projects (members):** Project **edit** — **Add from directory** uses **`GET /api/directory`** + **`DirectoryParticipantPicker`** (same pattern as tasks/events).
...
- [x] **Schema + RPC — Message Center / MAG messaging (214):** `mags.allow_conversations`; `crm_contacts.mag_community_messaging_enabled`; `crm_contact_mag_community_opt_in`; Customizer seeds `message_center_category`, `message_center_timeline_kind`, `message_center_thread_type`; MAG dynamic RPCs return `allow_conversations`.
- [x] **Read APIs — admin dashboard Message Center:** `getAdminMessageCenterStream` (`src/lib/message-center/admin-stream.ts`); `GET /api/admin/message-center` (filter + limit); `GET /api/admin/message-center/unread` (thread participant unread); `PATCH /api/conversation-threads/[threadId]/read`. Thread-head rows ∪ timeline (+ blog + synthetic CRM lines); `form_submission` → `form_submitted` read-side alias in stream. **2026-03-26:** Support-thread **`threadSearchText`** enrichment for search; omit duplicate timeline **`kind: message`** rows when a **support** thread for the same contact is already in the merged stream.
- [x] **MAG thread policy (API):** `assertCanPostThreadMessage` — broadcast-only when `allow_conversations` false (superadmin + tenant admin only; no `author_contact_id`); community path when on (**staff** any admin role; **members** with `members` row + matching `author_contact_id` + **enrolled in MAG** — aligned with read gate; **2026-03-28** dropped separate global + per-MAG opt-in for **`mag_group` POST**). Wired on **POST** thread messages + bootstrap `first_message`. **`GET .../messages`:** strip **`metadata.source === admin_broadcast`** from **`mag_group`** transcript for **non-staff** readers.
- [x] **Pattern A — support/task thread participants:** On create of support / `task_ticket` thread from `ensureThreadForNote`, seed `thread_participants` for all auth users assigned to the tenant site (`thread-participants.ts`, `listAuthUserIdsAssignedToTenantSite`).
- [x] **Read APIs — GPUM (v1):** `getMemberMessageCenterMergedStream`, `GET /api/members/message-center` — `streamItems` + legacy `items`; **`admin_only`** excluded via **`getMemberActivity`**. **Keyset pagination:** `cursor` / **`nextCursor` / `hasMore`** (`gpum-message-center-pagination.ts`); legacy **`items`** empty when **`cursor`** is set.
- [x] **UI — contact detail Message Center:** **`DashboardActivityStream`** on contact **Message Center** tab (`ContactRecordLayout`) + **Open full Message Center** link with `contact_id`; server `getAdminMessageCenterStream` in `contacts/[id]/page.tsx`. **Notes tab:** `ContactNotificationsTimelineSection` (list + add `staff_note`) remains separate.
- [x] **UI — admin dashboard Message Center:** **`DashboardTabsClient`** tab + **`DashboardActivityStream`** (filters, server fetch per filter, blog moderation hooks); replaces legacy flat `getDashboardActivity` for this tab.
- [x] **UI — GPUM Messages / notifications (v1):** **`MemberActivityStream`** on **`/members/messages`** — merged stream, filters, date presets, inline thread transcript + reply; **Join a conversation** / **Conversations** / **Message the team** / **View all**; conversation heads for support + eligible MAGs. **2026-03-27:** **`unread`** on **`conversation_head`** (`getMemberThreadUnreadMapForUser` + transcript load triggers **`PATCH /api/conversation-threads/[threadId]/read`**). **2026-03-27:** **`MemberActivityStream`** empty states — no linked member, **All activity / Conversations / Notifications**, date range, search miss, empty transcript copy. **`MemberMessagesPreview`** on **`/members`** dashboard (**See all** → **`/members/messages`**). **2026-03-27:** **Load more** + **`cursor` / `nextCursor`** alignment with **`GET /api/members/message-center`**. **Phase 3.4 sheet picker:** **not shipped by design** — stream UX replaces it ([sessionlog §3](./sessionlog.md), [plan-gpum-message-center-mvp.md](./reference/plan-gpum-message-center-mvp.md) **3.4**).

**COMMENT:GROUP — MAG shared group thread (next feature)** — **Product lock:** [messages-and-notifications-wiring.md § COMMENT:GROUP](./reference/messages-and-notifications-wiring.md#commentgroup-mag-shared-thread-vs-announcements). One **`thread_type = mag_group`** room **per MAG** (all members in the MAG see the same thread; not per-person). **Announcements** (broadcast) stay **separate**: scoped **single-line / splash** timeline behavior; **not required** for the group thread to exist or show for GPUMs. **Trust model:** unmoderated in-thread; **admin conversation toggle** on membership can **disable** the thread if needed; **admin** identifies commenters and may **open the contact** for person-level follow-up. **GPUM** sees **who** posted; **no** click-through to DM other members.

- [x] **Copy / labels — admin + GPUM:** Primary row label **COMMENT:GROUP** (admin stream + GPUM primary lines + **`MagGroupCommentsPanel`**); MAG detail toggle copy **Allow comments for this membership access group** (**2026-03-28**).
- [x] **Thread lifecycle:** **`getOrCreateMagGroupThread`** ensures a **`mag_group`** row when building GPUM **`conversation_head`**s for enrolled + **`allow_conversations`** MAGs; admin **Comments** tab + first post still create/use the same room; broadcasts do not define thread existence (**2026-03-28**).
- [x] **Admin — MAG / membership detail:** **Comments** tab + **`MagGroupCommentsPanel`** — **`mag_group`** transcript (conversation layout: member left muted/violet chip, staff right blue); announcements excluded from panel list; toggle **`allow_conversations`** (**2026-03-28** polish).
- [x] **Admin — Message Center:** **`mag_group`** non-broadcast rows as **COMMENT:GROUP** + violet list chip; **click → `/admin/crm/memberships/{id}?tab=comments`**; rolled-up transcript uses **same slate/blue** as other threads (member left muted, staff right blue) (**2026-03-28**).
- [ ] **Admin — Message Center — announcements:** Drill-down stays **read-focused** (full text + details); **omit or hide MAG audience picker** on read-only announcement view if it only mattered at compose time.
- [ ] **Admin — transcript attribution:** Render **contact name** and **display name** (when resolved); **click author → `/admin/crm/contacts/[id]`** (or current contact canonical route).
- [ ] **GPUM — transcript:** Show **same attribution** for context; **no** links that open **DM** or another member’s profile for unsolicited 1:1.
- [x] **API / policy pass (2026-03-28):** **`mag-thread-policy.ts`** POST/read aligned with enrollment + **`allow_conversations`**; **`memberCanSeeMagGroupCommentHead`** drives GPUM **COMMENT:GROUP** stream visibility (replaces strict **`memberCanSeeMagGroupThread`** for listing); **`GET`/`POST`** **`/api/conversation-threads/[threadId]/messages`** verified for member vs staff.
- [ ] **QA / docs:** Extend [qa-gpum-message-center-phase-53.md](./qa-gpum-message-center-phase-53.md) + wiring checkboxes; trim [sessionlog.md](./sessionlog.md) if this replaces an open “MAG thread visibility” confusion.
...
- [ ] **Personal capture inbox (webhook):** Per-tenant-user `POST` JSON from external apps (e.g. voice transcription); **admin-first**; tenant admin controls whether **non-admin** roles may enable an endpoint; inbox rows **private to recipient**; surface in **Message Center** + triage actions + statuses (processed / archived / complete). **Planned steps:** [sessionlog §3](./sessionlog.md#personal-capture-inbox-webhook-planned).
...
- **Taxonomy (locked):** Use the **existing taxonomy system** for projects. Add **Projects** as a taxonomy section (like Post, Page, CRM, Media). Projects link to this section for **categories** and **tags**. *Admin list/detail status and primary “project type” dimension (2026-03):* stored as **Customizer slugs** on the row (`project_status_slug`, `project_type_slug`); migration **205** removed `status_term_id` / `project_type_term_id`. Taxonomy **Projects** section remains for **categories/tags** assignment on **edit**.
...
- [ ] **Project Management (projects + tasks)** — step-by-step. Feature slug: **projects** (roles/gate step at end).
  - [x] **Schema — projects:** Table `projects`: id, name, description, **project_status_slug**, **project_type_slug** (**205** — Customizer), **estimated_hourly_rate** (**203**), cover image (**204**), start_date, due_date, completed_date (nullable dates), end_date_extended (boolean), planned_time (**201**/**202**), potential_sales, required_mag_id (FK → mags), contact_id, client_organization_id, archived_at, **project_number** (**200**), timestamps, created_by. RLS, indexes. **199** renames former `proposed_*` dates and adds `completed_date`; run SQL on each tenant.
  ...
  - [x] **RPC/API — projects:** List (filter **`project_status_slug`**, MAG, taxonomy, assignment), get by id, create, update, delete. Tenant + MAG visibility. **205** — `get_projects_dynamic` / `get_project_by_id_dynamic` slug columns + **`project_status_slug_filter`**.
  - [x] **RPC/API — tasks:** List by project / assignment / "my tasks", filter by taxonomy (category/tag for Task section — phase/milestone). Get by id, create, update, delete. Auto-extend project `due_date` when task `due_date` > project `due_date` (set `end_date_extended`; optionally update project `due_date`).
  ...
  - [x] **Admin UI — projects list (refresh):** Table: title + project-type color dot, Due date column, client (contact/org link + avatar), status pill, member avatars (contact/team), task-segment progress bar (done/overdue/todo/cancelled), project type. Batch server data (members, tasks, contacts, orgs, profiles); include_archived filter. Migration 172 adds avatar_url to crm_contacts and organizations (tenant schema).
  ...
  - [x] **Admin UI — project detail (layout refresh, Mar 2026):** Donor-style overview + stat cards; bottom tabs **Message Center** (shell) · Tasks · Events · Transactions · Attachments (shell) · **Deliverables** (shell); Activities breadcrumb; **`ProjectDetailClient.tsx`** — changelogs **2026-03-23** and **2026-03-25**.
  - [ ] **Admin UI — project detail Attachments:** Replace placeholder tab with uploads or document links (schema/API TBD).
  - [x] **Admin UI — project create/edit:** Form: name, description, **Customizer** status/type slugs, start/due/completed dates, potential_sales, required_mag_id, taxonomy (categories/tags), **estimated hourly rate**, **cover image**. **Edit (Mar 2026):** Save requires a **client**; **Client & members** 25%/75%; **Set client**; **Add from directory**; header **Cancel** / **Save** (`form="project-edit-form"`).
  ...
  - [x] **Schema / RPC — tasks `contact_id` (migration 207):** FK to **`crm_contacts`**; **`get_tasks_dynamic` / `get_task_by_id_dynamic`** return **`contact_id`**. **Applied** on primary Supabase (2026-03-25); other forks run **`207_tasks_contact_id.sql`** in SQL Editor when deploying.
  - [x] **Schema / RPC — tasks nullable `project_id` (migration 208):** Loose tasks; **`get_tasks_dynamic`** LEFT JOIN **`projects`**. **Applied** on primary after **207**; other forks run **`208_tasks_project_id_nullable.sql`** when deploying.
  - [x] **Admin UI — tasks (contact + optional project, Mar 2026 evening):** Assignees card — **linked CRM contact** (directory picker, single-select contact); **project** on edit (Phase card first) including **No project**; canonical **`/admin/projects/tasks/[taskId]`** detail + **`/edit`**; **All Tasks** **No project** links/group; **`taskDetailPath` / `taskEditPath`**; task edit header **Cancel** then **Save**.
  ...
  - [x] **QA — All Tasks (manual):** Default paint; preset + modal narrow results; **Overdue** + title search; column sort vs completed visibility; master reset → recap (session **2026-03-25**).
...
- **Phase 19 expansion (decisions 3/16/2026):** Orders + invoices remain two tables; project_id on both; project total = sum(orders.total). Activity stream now targets contact timeline + conversation threads/thread_messages (legacy crm_notes design superseded); focus-mode UI later. User handle for messaging/conversations.
  - [ ] **Schema — task start_date:** Add start_date (DATE, nullable) to tasks. Migration; RPC/types; task forms and list/detail.
  - [x] **Schema — task_id on crm_notes:** **Legacy/superseded** by `conversation_threads` + `thread_messages` + contact timeline cutover (kept table only for temporary compatibility/backfill review).
  - [x] **Schema — conversation_uid on crm_notes:** **Legacy/superseded** by thread subject model + thread ids; no new runtime dependency.
...
- **Phase 19 — Project members, client, assignee scoping (plan):** See [project-members-and-client-plan.md](./project-members-and-client-plan.md). **Core PM taxonomy sections (non-deletable):** Project Type, Project Status, Task Type, Task Status, Task Priority, **Project Roles** (new). Reserved terms (e.g. Support, Perpetual) protected from deletion (optional this phase).
  ...
  - [x] **Schema — project_members:** Table project_members (project_id, user_id | contact_id, **role_slug** text — **206** dropped `role_term_id`); RLS; list/add/remove API with **`role_slug`** (Customizer **`project_role`**).
  - [x] **Project UI — client:** Client = Contact or Organization; when org, “Add org members” (all or tick) → project_members. **Edit:** Client set only via **Set client** (not mixed into Add member).
