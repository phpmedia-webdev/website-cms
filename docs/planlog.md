...
**Status:** In progress — admin **Message Center** v1 + **CRM contact** scoped stream + full-page **`/admin/dashboard/message-center`** shipped (**214** + app). **GPUM** merged stream + **`/members/messages`** (`MemberActivityStream`) shipped at **functional v1**; **open:** member thread **unread**/mark-read, cursor pagination on `GET /api/members/message-center` (stub), wiring doc + QA, cutover/edge-case checklist. Spec alignment: [prd.md — Shared identity UX (forks)](./prd.md#shared-identity-ux-forks) (auth unchanged). **No CRM history backfill required in dev** unless desired later.
...
- [x] **MAG thread policy (API):** `assertCanPostThreadMessage` — broadcast-only when `allow_conversations` false (superadmin + tenant admin only; no `author_contact_id`); community path when on (staff or GPUM with global + per-MAG opt-in). Wired on **POST** thread messages + bootstrap `first_message`.
...
- [x] **Read APIs — GPUM (v1):** `getMemberMessageCenterMergedStream`, `GET /api/members/message-center` — `streamItems` + legacy `items`; **`admin_only`** excluded via **`getMemberActivity`**. **Open:** real **`nextCursor` / `hasMore`** pagination (currently stubbed).
...
- [x] **UI — GPUM Messages / notifications (v1):** **`MemberActivityStream`** on **`/members/messages`** — merged stream, filters, date presets, inline thread transcript + reply; **`Message support`** + conversation heads for support + eligible MAGs. **Open:** unread/mark-read, empty states, `/members` dashboard preview, optional sheet picker for new MAG thread ([sessionlog §3](./sessionlog.md), [plan-gpum-message-center-mvp.md](./reference/plan-gpum-message-center-mvp.md)).
