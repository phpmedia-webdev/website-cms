...
/** Content type slugs hidden from main Content list and New content picker (e.g. Product under Ecommerce → Products; Template under Marketing → Templates). */
export const CONTENT_LIST_EXCLUDED_TYPE_SLUGS = ["product", "template"];
 ...
 * Also used for template lookup (type "template") by slug for transactional email.
 */
export async function getContentByTypeAndSlug(
  typeSlug: "post" | "article" | "template",
  slug: string
): Promise<ContentRow | null> {
  const { createClient } = await import("@supabase/supabase-js");
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
