...
import { getEffectiveFeatureSlugs, getRoleFeatureSlugsForGating } from "@/lib/supabase/feature-registry";
 ...
 * - Tenant user: returns slug[] from getRoleFeatureSlugsForGating(role) (PHP-Auth when configured, else local).
...
export async function getRoleFeatureSlugsForCurrentUser(): Promise<string[] | "all"> {
  ...
  if (!role) return [];
  if (role === PHP_AUTH_ROLE_SLUG.SUPERADMIN) return "all";
  return getRoleFeatureSlugsForGating(role);
}
