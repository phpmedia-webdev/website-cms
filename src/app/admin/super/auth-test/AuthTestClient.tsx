...
import { CheckCircle2, XCircle, Loader2, Play, Send, List, ChevronDown, ChevronUp, ShieldCheck } from "lucide-react";

export type VerifySessionCheck = { label: string; ok: boolean; detail?: string };
...
interface AuthTestClientProps {
  verifySession: {
    checks: VerifySessionCheck[];
    allOk: boolean;
    phpAuthConfigured: boolean;
    devBypass2FA: boolean;
  };
}

export function AuthTestClient({ verifySession }: AuthTestClientProps) {
  const { checks, allOk, phpAuthConfigured, devBypass2FA } = verifySession;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Verify session, then test endpoints against the live PHP-Auth app ({LIVE_BASE}). Uses your current session and env (AUTH_BASE_URL, AUTH_API_KEY).
        </p>
      </div>
      ...
      <Tabs defaultValue="verify-session" className="w-full">
        <TabsList className="grid w-full max-w-lg grid-cols-3">
          <TabsTrigger value="verify-session" className="gap-2">
        ...
        <TabsContent value="verify-session" className="space-y-6 mt-4">
          <Card>
