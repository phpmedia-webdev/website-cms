...
export default async function BlogCategoryArchivePage({ params, searchParams }: Props) {
  const { slug } = await params;
  const termSlug = decodeURIComponent(slug);
  const term = await getTermBySlugAndType(termSlug, "category");
  if (!term) notFound();
  ...
  return (
    <main className="container mx-auto px-4 py-16">
      ...
      <h1 className="text-4xl font-bold mb-8">Category: {term.name}</h1>
      {posts.length === 0 ? (
