-- File: 146_projects_taxonomy_section.sql
-- Phase 19: Add Projects as a core taxonomy section (is_staple = true, not deletable).
-- Allow content_type = 'project' in taxonomy_relationships for linking projects to categories/tags.
...
-- 1. Add Projects section (core; cannot be deleted in Taxonomy settings)
...
VALUES ('project', 'Projects', 'project', NULL, NULL, true)
...
-- 2. Allow content_type = 'project' in taxonomy_relationships (drop and re-add constraint)
...
BEGIN
  ALTER TABLE website_cms_template_dev.taxonomy_relationships
    ...
    CHECK (
      content_type IN ('post', 'page', 'media', 'gallery', 'event', 'crm_contact', 'template', 'project')
